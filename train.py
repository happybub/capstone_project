...
def train_epoch_DEBUG(net, optim, dataloader_map, config, epoch, mode='train'):
    print_time = config['PRINT_TIME']
    net.print_time = print_time
    if mode == 'train':
        dataloader = dataloader_map['train']
    ...
    for i, images in enumerate(dataloader):
        ...
        with torch.set_grad_enabled(mode == 'train'):
            ...
            freq_noise_prob = freq_noise / freq_noise.sum()
            r_freq_noise_prob = r_freq_noise / r_freq_noise.sum()

            eps = 1e-10
            freq_noise_prob = torch.clamp(freq_noise_prob, eps, 1)
            r_freq_noise_prob = torch.clamp(r_freq_noise_prob, eps, 1)

            kl_divergence = torch.nn.functional.kl_div(
                freq_noise_prob.log(), r_freq_noise_prob, reduction='batchmean'
            )
            ...
            total_loss = lambda_image_loss * image_loss + lambda_secret_loss * secret_loss + lambda_distribution_loss * kl_divergence
        ...
        print(f'Batch: #{i}, Image Loss: {image_loss.item()}, Secret Loss: {secret_loss.item()}, KL divergence: {kl_divergence.item()}, Total Loss: {total_loss}')
...
def train(name, start_epoch, end_epoch, config):
    ...
    for epoch in range(start_epoch, end_epoch):
        ...
        train_epoch_DEBUG(net, optim, dataloader_map, config, epoch, mode='train')

        # validate the model
        train_epoch_DEBUG(net, optim, dataloader_map, config, epoch, mode='val')
